...
var (
	appBackupDir   string
	SystemWidePath = "/Applications/Psyllama.app"
)
...
func init() {
	...
	// Executable = Psyllama.app/Contents/MacOS/Psyllama

	UpgradeLogFile = filepath.Join(home, ".psyllama", "logs", "upgrade.log")
	...
	appDataDir := filepath.Join(cacheDir, "psyllama")
...
func DoUpgrade(interactive bool) error {
	...
	appBackup := filepath.Join(appBackupDir, "Psyllama.app")
	...
	// Bundle contents Psyllama.app/Contents/...
	links := []*zip.File{}
	...
	for _, f := range links {
		s := strings.SplitN(f.Name, "/", 2) // Strip off Psyllama.app/
		if len(s) < 2 || s[1] == "" {
		...
		// Don't allow links outside of Psyllama.app
...
func verifyDownload() error {
	...
	dir, err := os.MkdirTemp("", "psyllama_update_verify")
	if err != nil {
		return err
	}
	...
	if err := verifyExtractedBundle(filepath.Join(dir, "Psyllama.app")); err != nil {
