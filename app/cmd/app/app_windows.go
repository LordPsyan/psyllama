...
import (
	...
	"github.com/LordPsyan/psyllama/app/wintray"
...
var (
	...
	appPath         = filepath.Join(os.Getenv("LOCALAPPDATA"), "Programs", "Psyllama")
	appLogPath      = filepath.Join(os.Getenv("LOCALAPPDATA"), "Psyllama", "app.log")
	startupShortcut = filepath.Join(os.Getenv("APPDATA"), "Microsoft", "Windows", "Start Menu", "Programs", "Startup", "Psyllama.lnk")
	psyllamaPath      string
	DesktopAppName  = "psyllama app.exe"
)
...
func init() {
	...
	psyllamaPath = filepath.Join(appPath, "psyllama.exe")

	// Handle developer mode (go run ./cmd/app)
	if _, err := os.Stat(psyllamaPath); err != nil {
		pwd, err := os.Getwd()
		if err != nil {
			slog.Warn("missing psyllama.exe and failed to get pwd", "error", err)
			return
		}
		distAppPath := filepath.Join(pwd, "dist", "windows-"+runtime.GOARCH)
		distPsyllamaPath := filepath.Join(distAppPath, "psyllama.exe")
		if _, err := os.Stat(distPsyllamaPath); err == nil {
			slog.Info("detected developer mode")
			appPath = distAppPath
			psyllamaPath = distPsyllamaPath
		}
	}
}
...
func osRun(shutdown func(), hasCompletedFirstRun, startHidden bool) {
	...
	if !startHidden {
		...
		// ~\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\Psyllama
...
func createLoginShortcut() error {
	...
	shortcutOrigin := filepath.Join(appPath, "lib", "Psyllama.lnk")
...
func logStartup() {
	slog.Info("starting Psyllama", "app", appPath, "version", version.Version, "OS", updater.UserAgentOS)
}
...
func runInBackground() {
	...
	if cmd != nil {
		err = cmd.Run()
		if err != nil {
			slog.Error("failed to run Psyllama", "exe", exe, "error", err)
			os.Exit(1)
		}
	} else {
		slog.Error("failed to start Psyllama", "exe", exe)
		os.Exit(1)
	}
}
