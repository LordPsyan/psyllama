...
- (void)application:(NSApplication *)application openURLs:(NSArray<NSURL *> *)urls {
    for (NSURL *url in urls) {
        if ([url.scheme isEqualToString:@"psyllama"]) {
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    if (![bundlePath hasSuffix:@".app"]) {
        ...
        NSString *iconPath = [cwdPath
            stringByAppendingPathComponent:
                [NSString
                    stringWithFormat:
                        @"darwin/Psyllama.app/Contents/Resources/icon.icns"]];
    ...
    NSMenuItem *openMenuItem =
        [[NSMenuItem alloc] initWithTitle:@"Open Psyllama"
    ...
    [menu addItemWithTitle:@"Quit Psyllama"
...
- (void)openHelp:(id)sender {
    NSURL *url = [NSURL URLWithString:@"https://docs.psyllama.com/"];
    ...
    if (![bundle.bundlePath hasSuffix:@".app"]) {
        ...
        NSString *bundlePath =
            [cwdPath stringByAppendingPathComponent:
                         [NSString stringWithFormat:@"darwin/Psyllama.app"]];
...
- (void)registerSelfAsLoginItem:(BOOL)firstTimeRun {
    ...
    // Maps to the file Psyllama.app/Contents/Library/LaunchAgents/com.psyllama.psyllama.plist
    SMAppService* service = [SMAppService agentServiceWithPlistName:@"com.psyllama.psyllama.plist"];
    if (!service) {
        appLogInfo(@"SMAppService failed to find service for com.psyllama.psyllama.plist");
        return;
    }
...
/// Remove psyllama from the deprecated Login Items list as we now use LaunchAgents
- (void)unregisterSelfFromLoginItem {
    ...
    for (id item in (__bridge NSArray *)currentItems) {
        ...
        if (LSSharedFileListItemResolve((LSSharedFileListItemRef)item, 0,
            ...
            // Compare the prefix to match against "keep existing" flow, e.g. // "/Applications/Psyllama.app" vs "/Applications/Psyllama 2.app"
...
+ (void)swizzleWKWebViewContextMenu {
    ...
    SEL swizzledSelector = @selector(psyllama_willOpenMenu:withEvent:);
...
@implementation WKWebView (PsyllamaContextMenu)
- (void)psyllama_willOpenMenu:(NSMenu *)menu withEvent:(NSEvent *)event {
    [self psyllama_willOpenMenu:menu withEvent:event];
...
// instance of Psyllama is running
void killOtherInstances() {
    ...
    for (NSRunningApplication *app in apps) {
        ...
        if ([bundleId isEqualToString:[[NSBundle mainBundle] bundleIdentifier]] ||
            [bundleId isEqualToString:@"ai.psyllama.psyllama"] ||
            [bundleId isEqualToString:@"com.electron.psyllama"]) {
            
            pid_t pid = app.processIdentifier;
            if (pid != myPid && pid > 0) {
                appLogInfo([NSString stringWithFormat:@"terminating other psyllama instance %d", pid]);
                kill(pid, SIGTERM);
            } else if (pid == -1) {
...
AuthorizationRef getSymlinkAuthorization() {
    return getAuthorization(@"Psyllama is trying to install its command line "
                            @"interface (CLI) tool.",
                            @"symlink");
}
...
bool moveToApplicationsWithAuthorization(const char *src) {
    ...
    // Remove existing /Applications/Psyllama.app (if any)
...
enum AppMove askToMoveToApplications() {
    ...
    if (!evt || [evt eventID] != kAEOpenApplication) {
        ...
        // or the CLI spawns the app via open -a Psyllama.app
        appLogDebug([NSString
    ...
    [alert setInformativeText:
               @"Psyllama works best when run from the Applications directory."];
...
int installSymlink(const char *cliPath) {
    NSString *linkPath = @"/usr/local/bin/psyllama";
...
void styleWindow(uintptr_t wndPtr) {
    ...
    if (w.toolbar == nil) {
        NSToolbar *tb = [[NSToolbar alloc] initWithIdentifier:@"PsyllamaToolbar"];
