...
// to save and load data such as psyllama server configuration, messages,
...
type Settings struct {
	// Expose is a boolean that indicates if the psyllama server should
	// be exposed to the network
	Expose bool

	// Browser is a boolean that indicates if the psyllama server should
	...
	// inference information to be shared with Psyllama
	Survey bool
	...
	// ContextLength specifies the context length for the psyllama server (using PSYLLAMA_CONTEXT_LENGTH)
	ContextLength int

	// TurboEnabled indicates if Psyllama Turbo features are enabled
	TurboEnabled bool
...
var defaultDBPath = func() string {
	switch runtime.GOOS {
	case "windows":
		return filepath.Join(os.Getenv("LOCALAPPDATA"), "Psyllama", "db.sqlite")
	case "darwin":
		return filepath.Join(os.Getenv("HOME"), "Library", "Application Support", "Psyllama", "db.sqlite")
	default:
		return filepath.Join(os.Getenv("HOME"), ".psyllama", "db.sqlite")
	}
}()
...
var legacyConfigPath = func() string {
	switch runtime.GOOS {
	case "windows":
		return filepath.Join(os.Getenv("LOCALAPPDATA"), "Psyllama", "config.json")
	case "darwin":
		return filepath.Join(os.Getenv("HOME"), "Library", "Application Support", "Psyllama", "config.json")
	default:
		return filepath.Join(os.Getenv("HOME"), ".psyllama", "config.json")
	}
}()
...
// After this, cloud state is sourced from server.json OR PSYLLAMA_NO_CLOUD.
...
func (s *Store) Settings() (Settings, error) {
	...
	if settings.Models == "" {
		...
		if dir != "" {
			...
			if err == nil {
				settings.Models = filepath.Join(home, ".psyllama", "models")
			}
		}
	}
