...
func TestCloudMigrationFromAirplaneMode(t *testing.T) {
	...
	configPath := filepath.Join(tmpHome, ".psyllama", serverConfigFilename)
	...
	if cfg["disable_psyllama_cloud"] != true {
		t.Fatalf("disable_psyllama_cloud = %v, want true", cfg["disable_psyllama_cloud"])
	}
