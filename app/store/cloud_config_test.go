...
func TestSetCloudEnabled(t *testing.T) {
	...
	configPath := filepath.Join(configDir, serverConfigFilename)
	if err := os.WriteFile(configPath, []byte(`{"another_key":"value","disable_psyllama_cloud":true}`), 0o644); err != nil {
	...
	if got["disable_psyllama_cloud"] != false {
		t.Fatalf("disable_psyllama_cloud = %v, want false", got["disable_psyllama_cloud"])
	}
