...
// with the psyllama service. The methods of the [Client] type correspond to
// the psyllama REST API as described in [the API documentation].
// The psyllama command-line client itself uses this package to interact with
// the backend service.
//
// # Examples
//
...
// repository].
//
// [the API documentation]: https://github.com/LordPsyan/psyllama/blob/main/docs/api.md
// [in the GitHub repository]: https://github.com/LordPsyan/psyllama/tree/main/api/examples
package api
...
// Client encapsulates client state for interacting with the psyllama
...
// environment variable PSYLLAMA_HOST, which points to the network host and
// port on which the psyllama service is listening. The format of this variable
// is:
//
//	<scheme>://<host>:<port>
//
// If the variable is not specified, a default psyllama host and port will be
// used.
...
func (c *Client) do(ctx context.Context, method, path string, reqData, respData any) error {
	...
	var token string
	if envconfig.UseAuth() || c.base.Hostname() == "psyllama.com" {
	...
	request.Header.Set("User-Agent", fmt.Sprintf("psyllama/%s (%s %s) Go/%s", version.Version, runtime.GOARCH, runtime.GOOS, runtime.Version()))
...
// Pull downloads a model from the psyllama library. fn is called each time
...
// Push uploads a model to the model library; requires registering for psyllama.com
...
//
// [Modelfile]: https://github.com/LordPsyan/psyllama/blob/main/docs/modelfile.mdx
...
// Version returns the Psyllama server version as a string.
...
// Signout will signout a client for a local psyllama server.
func (c *Client) Signout(ctx context.Context) error {
	return c.do(ctx, http.MethodPost, "/api/signout", nil, nil)
}

// Disconnect will disconnect an psyllama instance from psyllama.com.
