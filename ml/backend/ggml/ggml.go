...
import (
	...
	fsggml "github.com/LordPsyan/psyllama/fs/ggml"
...
func (t *Tensor) SSMScan(ctx ml.Context, x, dt, A, B, C_, ids ml.Tensor) ml.Tensor {
	return &Tensor{
		b: t.b,
		t: C.ggml_ssm_scan(ctx.(*Context).ctx, t.t, x.(*Tensor).t, dt.(*Tensor).t, A.(*Tensor).t, B.(*Tensor).t, C_.(*Tensor).t, ids.(*Tensor).t),
	}
}
