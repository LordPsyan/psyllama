...
static void psyllama_debug_tensor(const struct ggml_tensor *tensor, bool verbose, const char *prefix, int indent) {
...
void psyllama_debug(const struct ggml_tensor *tensor, bool verbose) {
    psyllama_debug_tensor(tensor, verbose, ">>> ", 4);
    ...
    for (int i = 0; i < GGML_MAX_SRC && tensor->src[i] != NULL; ++i) {
        ...
        psyllama_debug_tensor(tensor->src[i], verbose, src, 4);
    }
}
