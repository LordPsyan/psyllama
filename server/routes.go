...
import (
	...
	"github.com/LordPsyan/psyllama/server/internal/registry"
	"github.com/LordPsyan/psyllama/template"
	"github.com/LordPsyan/psyllama/thinking"
	"github.com/LordPsyan/psyllama/tools"
	...
	imagegenmanifest "github.com/LordPsyan/psyllama/x/imagegen/manifest"
	xserver "github.com/LordPsyan/psyllama/x/server"
)

const signinURLStr = "https://psyllama.com/connect?name=%s&key=%s"
...
func experimentEnabled(name string) bool {
	return slices.Contains(strings.Split(os.Getenv("PSYLLAMA_EXPERIMENT"), ","), name)
}
...
func (s *Server) scheduleRunner(ctx context.Context, name string, caps []model.Capability, requestOpts map[string]any, keepAlive *api.Duration) (llm.LlamaServer, *Model, *api.Options, error) {
	...
	if slices.Contains(model.Config.ModelFamilies, "mllama") && len(model.ProjectorPaths) > 0 {
		return nil, nil, nil, fmt.Errorf("'llama3.2-vision' is no longer compatible with your version of Psyllama and has been replaced by a newer version. To re-download, run 'psyllama pull llama3.2-vision'")
	}
...
func (s *Server) GenerateHandler(c *gin.Context) {
	...
	if !req.Raw {
		...
		var b bytes.Buffer
		if req.Context != nil {
			slog.Warn("the context field is deprecated and will be removed in a future version of Psyllama")
...
func GetModelInfo(req api.ShowRequest) (*api.ShowResponse, error) {
	...
	var sb strings.Builder
	fmt.Fprintln(&sb, "# Modelfile generated by \"psyllama show\"")
...
func (s *Server) GenerateRoutes(rc *psyllama.Registry) (http.Handler, error) {
	...
	// General
	r.HEAD("/", func(c *gin.Context) { c.String(http.StatusOK, "Psyllama is running") })
	r.GET("/", func(c *gin.Context) { c.String(http.StatusOK, "Psyllama is running") })
...
func Serve(ln net.Listener) error {
	...
	slog.Info(fmt.Sprintf("Psyllama cloud disabled: %t", cloudDisabled))
	...
	var rc *psyllama.Registry
	if useClient2 {
		var err error
		rc, err = psyllama.DefaultRegistry()
		if err != nil {
			return err
		}
	}
...
func (s *Server) WhoamiHandler(c *gin.Context) {
	...
	u, err := url.Parse("https://psyllama.com")
	if err != nil {
