# Image Generation in Psyllama (Experimental)
...
```bash
# Run with a prompt
psyllama run z-image "a sunset over mountains"
Generating: step 30/30
Image saved to: /tmp/psyllama-image-1704067200.png
```
...
```bash
# Generate an image
psyllama run z-image "a cat playing piano"

# Check if model is running
psyllama ps

# Stop the model
psyllama stop z-image
```
...
- Psyllama built with MLX support
...
- Modelfiles not yet supported (use `psyllama create` from model directory)
...
The manifest follows the standard psyllama format with tensor-specific layer metadata:
...
```json
{
  ...
  "layers": [
    {
      "mediaType": "application/vnd.psyllama.image.tensor",
    ...
    {
      "mediaType": "application/vnd.psyllama.image.json",
...
```bash
cd ./weights/Z-Image-Turbo
psyllama create z-image-fp8 --quantize fp8
```
