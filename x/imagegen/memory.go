// Package imagegen provides experimental image generation capabilities for Psyllama.
//
// This package is in x/ because the tensor model storage format is under development.
// The goal is to integrate these capabilities into the main Psyllama packages once
...
// Checks both "architecture" (Psyllama format) and "_class_name" (diffusers format).
...
func DetectModelType(modelName string) string {
	...
	// Prefer architecture (Psyllama format), fall back to _class_name (diffusers)
