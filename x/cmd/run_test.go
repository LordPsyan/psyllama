...
func TestIsLocalServer(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "remote host",
			host:     "http://psyllama.example.com:11434",
			expected: true, // has :11434
		},
		{
			name:     "remote host different port",
			host:     "http://psyllama.example.com:8080",
			expected: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != tt.expected {
				t.Errorf("isLocalServer() with PSYLLAMA_HOST=%q = %v, expected %v", tt.host, result, tt.expected)
			}
		})
	}
}
