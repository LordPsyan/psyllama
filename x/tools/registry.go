...
// Tools returns all registered tools in Psyllama API format, sorted by name.
...
// - PSYLLAMA_AGENT_DISABLE_WEBSEARCH=1 disables web_search
// - PSYLLAMA_AGENT_DISABLE_BASH=1 disables bash
func DefaultRegistry() *Registry {
	r := NewRegistry()
	// TODO(parthsareen): re-enable web search once it's ready for release
	// if os.Getenv("PSYLLAMA_AGENT_DISABLE_WEBSEARCH") == "" {
	// 	r.Register(&WebSearchTool{})
	// }
	if os.Getenv("PSYLLAMA_AGENT_DISABLE_BASH") == "" {
		r.Register(&BashTool{})
	}
	return r
}
