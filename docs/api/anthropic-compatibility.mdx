...
Psyllama provides compatibility with the [Anthropic Messages API](https://docs.anthropic.com/en/api/messages) to help connect existing applications to Psyllama, including tools like Claude Code.

## Usage

### Environment variables

To use Psyllama with tools that expect the Anthropic API (like Claude Code), set these environment variables:

```shell
export ANTHROPIC_AUTH_TOKEN=psyllama  # required but ignored
...
client = anthropic.Anthropic(
    ...
    api_key='psyllama',  # required but ignored
)
...
const anthropic = new Anthropic({
  ...
  apiKey: "psyllama", // required but ignored
});
...
[Claude Code](https://code.claude.com/docs/en/overview) can be configured to use Psyllama as its backend. 
...
```shell
ANTHROPIC_AUTH_TOKEN=psyllama ANTHROPIC_BASE_URL=http://localhost:11434 claude --model qwen3-coder
```
...
Then run Claude Code with any Psyllama model:
...
Psyllama supports both local and cloud models.
...
For tooling that relies on default Anthropic model names such as `claude-3-5-sonnet`, use `psyllama cp` to copy an existing model name:

```shell
psyllama cp qwen3-coder claude-3-5-sonnet
```
