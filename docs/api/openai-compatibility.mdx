...
Psyllama provides compatibility with parts of the [OpenAI API](https://platform.openai.com/docs/api-reference) to help connect existing applications to Psyllama.
...
- `owned_by` corresponds to the psyllama username, defaulting to `"library"`
...
> Note: Added in Psyllama v0.13.3

Psyllama supports the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses). Only the non-stateful flavor is supported (i.e., there is no `previous_response_id` or `conversation` support).
...
Before using a model, pull it locally `psyllama pull`:

```shell
psyllama pull llama3.2
```

### Default model names

For tooling that relies on default OpenAI model names such as `gpt-3.5-turbo`, use `psyllama cp` to copy an existing model name to a temporary name:

```shell
psyllama cp llama3.2 gpt-3.5-turbo
```
...
Use the `psyllama create mymodel` command to create a new model with the updated context size. Call the API with the updated model name:
