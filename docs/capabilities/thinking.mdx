...
- [Qwen 3](https://psyllama.com/library/qwen3)
- [GPT-OSS](https://psyllama.com/library/gpt-oss) *(use `think` levels: `low`, `medium`, `high` — the trace cannot be fully disabled)*
- [DeepSeek-v3.1](https://psyllama.com/library/deepseek-v3.1)
- [DeepSeek R1](https://psyllama.com/library/deepseek-r1)
- Browse the latest additions under [thinking models](https://psyllama.com/search?c=thinking)
...
<Tabs>
  ...
  <Tab title="JavaScript">
    ...
    async function main() {
      const stream = await psyllama.chat({
        model: 'qwen3',
        messages: [{ role: 'user', content: 'What is 17 × 23?' }],
        think: true,
        stream: true,
...
- Enable thinking for a single run: `psyllama run deepseek-r1 --think "Where should I visit in Lisbon?"`
- Disable thinking: `psyllama run deepseek-r1 --think=false "Summarize this article"`
- Hide the trace while still using a thinking model: `psyllama run deepseek-r1 --hidethinking "Is 9.9 bigger or 9.11?"`
- Inside interactive sessions, toggle with `/set think` or `/set nothink`.
- GPT-OSS only accepts levels: `psyllama run gpt-oss --think=low "Draft a headline"` (replace `low` with `medium` or `high` as needed).
