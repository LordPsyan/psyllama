...
Psyllama supports tool calling (also known as function calling) which allows a model to invoke tools and incorporate their results into its replies.
...
<Tabs>
  ...
  <Tab title="Python">
    Install the Psyllama Python SDK:
    ```bash
    # with pip
    pip install psyllama -U
  ...
  <Tab title="JavaScript">
    Install the Psyllama JavaScript library:
    ```bash
    # with npm
    npm i psyllama
    ...
    if (response.message.tool_calls?.length) {
      ...
      const finalResponse = await psyllama.chat({ model: 'qwen3', messages, tools, think: true })
...
<Tabs>
  <Tab title="Python">
    ```python
    from psyllama import chat, ChatResponse
...
## Using functions as tools with Psyllama Python SDK
