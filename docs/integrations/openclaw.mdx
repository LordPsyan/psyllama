...
Psyllama handles everything automatically:

1. **Install** — If OpenClaw isn't installed, Psyllama prompts to install it via npm
...
4. **Onboarding** — Psyllama configures the provider, installs the gateway daemon, and sets your model as the primary
...
<Note>Previously known as Clawdbot. `psyllama launch clawdbot` still works as an alias.</Note>
...
```bash
psyllama launch openclaw --config
```

To use a specific model directly:

```bash
psyllama launch openclaw --model kimi-k2.5:cloud
```
...
More models at [psyllama.com/search](https://psyllama.com/search?c=cloud).
