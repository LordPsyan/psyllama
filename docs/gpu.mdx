...
## Nvidia
Psyllama supports Nvidia GPUs with compute capability 5.0+ and driver version 531 and newer.
...
If you have multiple NVIDIA GPUs in your system and want to limit Psyllama to use
...
On linux, after a suspend/resume cycle, sometimes Psyllama will fail to discover
...
Psyllama supports the following AMD GPUs via the ROCm library:
...
Psyllama leverages the AMD ROCm library, which does not support all AMD GPUs. In
...
Reach out on [Discord](https://discord.gg/psyllama) or file an
[issue](https://github.com/psyllama/psyllama/issues) for additional help.

### GPU Selection

If you have multiple AMD GPUs in your system and want to limit Psyllama to use a
...
Psyllama supports GPU acceleration on Apple devices via the Metal API.


## Vulkan GPU Support

> **NOTE:**
> Vulkan is currently an Experimental feature.  To enable, you must set PSYLLAMA_VULKAN=1 for the Psyllama server as
described in the [FAQ](faq#how-do-i-configure-psyllama-server)
...
For AMD GPUs on some Linux distributions, you may need to add the `psyllama` user to the `render` group.

The Psyllama scheduler leverages available VRAM data reported by the GPU libraries to
...
capability are granted, Psyllama will use approximate sizes of the models
to make best effort scheduling decisions.

```bash
sudo setcap cap_perfmon+ep /usr/local/bin/psyllama
```

### GPU Selection

To select specific Vulkan GPU(s), you can set the environment variable
`GGML_VK_VISIBLE_DEVICES` to one or more numeric IDs on the Psyllama server as
described in the [FAQ](faq#how-do-i-configure-psyllama-server). If you
